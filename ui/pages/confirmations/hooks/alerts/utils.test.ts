...
describe('Utils', () => {
  describe('getProviderAlertSeverity', () => {
    it.each([
      ...
      ['Other', Severity.Info],
    ])('maps %s to %s', (inputSeverity: BlockaidResultType | string, expectedSeverity: Severity) => {
      expect(getProviderAlertSeverity(inputSeverity as BlockaidResultType)).toBe(expectedSeverity);
    });
  });
  ...
  describe('normalizeProviderAlert', () => {
    ...
    it('normalizes a security alert response correctly', () => {
      ...
      expect(normalizedAlert.message).toBe('blockaidDescriptionMightLoseAssets');
    });
  });
});
